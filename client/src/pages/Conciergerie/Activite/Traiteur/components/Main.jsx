...
import traiteurData from "../TraiteurData";

export default function Main(props) {
  const { traiteurData } = props; // Assurez-vous que "activities" est défini
  ...
  return (
    <main className="main_activite">
      ...
      <section className="section_activite_contenu">
        {traiteurData &&
          traiteurData.map((traiteurData, index) => (
            ...
            <div className="activite_contenu" key={index}>
              <div className="activite_contenu_img">
                <img
                  src={traiteurData.image}
                  alt={`img_contenu_activite_${index}`}
                />
              </div>
              <div className="activite_contenu_txt">
                <h3>{traiteurData.title}</h3>
                <p>{traiteurData.description}</p>
                <button>
                  <Link to={traiteurData.link}>Voir plus</Link>
                </button>
              </div>
            </div>
          ))}
